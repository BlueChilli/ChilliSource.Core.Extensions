...
namespace Tests
{
	public class DateTimeExtensionTests
	{
        ...
        {
            var result = new DateTime(2001, 2, 28, 12, 33, 44).ThisMonth();
        ...
        {
            var result = DateTime.UtcNow.IsToday();
            Assert.Equal(true, result);

            result = DateTime.UtcNow.AddDays(-1).IsToday();
        ...
        {
            var result = DateTime.UtcNow.IsYesterday();
            Assert.Equal(false, result);

            result = DateTime.UtcNow.AddDays(-1).IsYesterday();
            Assert.Equal(true, result);
        ...
        {
            var result = DateTime.UtcNow.IsTomorrow();
            Assert.Equal(false, result);

            result = DateTime.UtcNow.AddDays(-1).IsTomorrow();
            Assert.Equal(false, result);
        ...
        {
            var result = DateTime.UtcNow.IsThisMonth();
            Assert.Equal(true, result);

            result = DateTime.UtcNow.AddMonths(-1).IsThisMonth();
            Assert.Equal(false, result);
